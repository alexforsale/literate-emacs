...
Since this file in /Emacs/ is considered to be a library, it should follow the [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Library-Headers.html][convention]]. But this is not needed in the /HTML/ exported file, so every heading with the name =header= or =footer= will be tagged with the =noexport= tag and won't show in the /HTML/ page, it's' kind of repetitive anyway.

* Wishlist
** Use =bookmarks= and =registers=
** Keybindings
I'm using =[[id:6efb5563-5427-47a2-a47f-f8424b47a80f][evil-mode]]=. So there will be keybindings that only available in certain modes, at the same time, I try as much as possible not to mess with the default /Emacs/ keybindings.
...
#+caption: general keybindings with =which-key=
[[./assets/img/general-keybinding-with-which-key.png][file:assets/img/general-keybinding-with-which-key.png]]
...
:ID:       6efb5563-5427-47a2-a47f-f8424b47a80f
:END:
...
(use-package evil-terminal-cursor-changer
  ...
  :config
  (defun +config/change-cursor-in-tty ()
    (unless (display-graphic-p)
      (evil-terminal-cursor-changer-activate)))
  (add-hook 'server-after-make-frame-hook '+config/change-cursor-in-tty))
#+end_src
** =evil-args=
...
** =doom-themes=
A megapack of themes for GNU Emacs[fn:15]. I'm not the kind of guy who enjoys /[[https://wiki.installgentoo.com/index.php/GNU/Linux_ricing][ricing]]/, I look every once in a while for a nice colorscheme and then I just stick it everywhere else. Been a /solarized/ fans most of the time, now I'm trying /[[https://www.nordtheme.com/][nord]]/.
#+begin_src emacs-lisp -n
...
#+end_src
** =doom-modeline=
A fancy and fast mode-line inspired by minimalism design[fn:16]. I'm using their themes, why not the modeline as well?
#+begin_src emacs-lisp -n
...
(use-package dashboard
  ...
  (setq initial-buffer-choice (lambda () (get-buffer-create "*dashboard*"))
        ...
        dashboard-filter-agenda-entry nil
        ...
        dashboard-agenda-sort-strategy
        '(todo-state-up time-down priority-down todo-state-down time-up priority-up)
        ))
#+end_src
...
#+end_src
** =helpful= :disabled:
Since /Emacs/ is a self-documenting editor[fn:18]. It's certainly nice to have a more contextual help buffers.
...
(use-package diff-hl
  ...
  (general-nvmap
    ...
    "[[" '(diff-hl-previous-hunk :wk "previous hunk" :repeat t :jump t)
    "]]" '(diff-hl-next-hunk :wk "next hunk" :repeat t :jump t)))
#+end_src
** =perspective=
...
(use-package eglot
  ...
  :bind (:map
           ...
           ("C-x c o" . eglot-code-actions-organize-imports)
...
(use-package treesit
  ...
  (setq treesit-language-source-alist
        '((bash . ("git@github.com:tree-sitter/tree-sitter-bash.git"))
          ...
          (common-lisp . ("git@github.com:tree-sitter-grammars/tree-sitter-commonlisp.git"))
          ...
          (julia . ("git@github.com:tree-sitter/tree-sitter-julia.git"))
          ...
          (perl . ("git@github.com:ganezdragon/tree-sitter-perl.git"))
...
:END:
My /org-mode/ configuration is [[https://alexforsale.github.io/posts/org-mode-workflow/][based around my workflow]].
** header :noexport:
#+begin_src emacs-lisp -n
...
(use-package toc-org
  ...
  (+config/local-leader
    ...
    "C-o" 'toc-org-markdown-follow-thing-at-point))
#+end_src
** =ox-pandoc= :disabled:
#+begin_src emacs-lisp -n :tangle no
...
"))))
  ...
  (+config/leader-notes
    ...
    "rdc" '(:ignore t :wk "capture day")
    "rdcd" 'org-roam-dailies-capture-date
    "rdct" 'org-roam-dailies-capture-today
    "rdg" '(:ignore t :wk "goto")
    "rdgt" 'org-roam-dailies-goto-today
    "rdgd" 'org-roam-dailies-goto-date
    "rdgp" 'org-roam-dailies-goto-previous-note
    "rdgy" 'org-roam-dailies-goto-yesterday
    "rdcy" 'org-roam-dailies-capture-yesterday
...
(use-package org-roam-ui
  ...
  :general
  (+config/leader-notes
    "ru" '(:ignore t :wk "org-roam-ui")
    "ruf" 'org-roam-ui-follow-mode
    "ruo" 'org-roam-ui-open
    "rus" 'org-roam-ui-sync-theme)
  (+config/leader-notes
    :keymaps 'org-roam-mode-map
    "ruz" 'org-roam-ui-node-zoom))
#+end_src
...
(use-package ob-ditaa
  :config
  (when (file-exists-p "/usr/share/java/ditaa/ditaa-0.11.jar")
    (setq org-ditaa-jar-path "/usr/share/java/ditaa/ditaa-0.11.jar")))
#+end_src
** =ob-diagrams= :disabled:
:PROPERTIES:
:ID:       f70ea00a-85fe-4d1c-b7c3-fcda13847bcc
:END:
...
*** requirements
- [[https://github.com/francoislaberge/diagrams][diagrams cli]]
- [[https://github.com/sverweij/state-machine-cat][state-machine-cat]]
...
#+caption: from the [[https://github.com/emacsmirror/khalel/][github page]]
#+begin_src emacs-lisp -n :tangle no
(use-package khalel
  :ensure
...
** =elfeed= :disabled:
#+caption: /elfeed/ RSS
[[./assets/img/emacs-elfeed-rss.png][file:assets/img/emacs-elfeed-rss.png]]
#+begin_src emacs-lisp -n :tangle no
(use-package elfeed
  :ensure
  :defer t
  ...
  (general-def
    ...
    "q" '(elfeed-kill-buffer :wk "kill buffer")
    "r" '(elfeed-search-update--force :wk "update")))
#+end_src
** =elfeed-org= :disabled:
#+begin_src emacs-lisp -n :tangle no
...
(use-package emms
  ...
  (emms-mpris-enable)
  (setq-default
   emms-source-playlist-formats 'm3u
  ...
  (general-nvmap
    ...
    "C-n" 'next-line
    "C-o" 'emms-playlist-mode-shift-track-down
    "C-p" 'previous-line
    "C-u" 'scroll-up
    "TAB" 'emms-playlist-mode-shift-track-up
    ...
    "q" 'bury-buffer)
  (+config/leader-open
    "e" '((lambda () (interactive) (require 'emms-browser) (emms-browser)) :wk "emms-browser")))
#+end_src
...
(use-package lorem-ipsum
  ...
  :general
  (+config/leader-insert
    "l" '(:ignore t :wk "insert lorem ipsum")
...
(use-package markdown-mode
  ...
  (+config/local-leader
    :keymaps 'markdown-mode-map
    "i" '(:ignore t :wk "insert")
    "d" '(markdown-do :wk "do checklist")
    "ib" '(markdown-insert-bold :wk "insert bold")
    "ic" '(markdown-insert-code :wk "insert code")
    "iC" '(markdown-insert-gfm-code-block :wk "insert gfm code block")
    "iF" '(markdown-insert-foldable-block :wk "insert foldable block")
    "if" '(markdown-insert-footnote :wk "insert footnote")
    "ig" '(markdown-insert-gfm-checkbox :wk "insert gfm checkbox")
    "ih" '(markdown-insert-header :wk "insert header")
    "iI" '(markdown-insert-image :wk "insert image")
    "ii" '(markdown-insert-italic :wk "insert italic")
    "ij" '(markdown-insert-list-item :wk "insert list items")
    "ik" '(markdown-insert-kbd :wk "insert kbd")
    "il" '(markdown-insert-link :wk "insert link")
    "ip" '(markdown-insert-pre :wk "insert pre")
    "ir" '(markdown-insert-hr :wk "insert horizontal ruler")
    "is" '(markdown-insert-strike-through :wk "insert strike-through")
    "it" '(markdown-insert-table :wk "insert table")
    "iu" '(markdown-insert-url :wk "insert url")
    "iw" '(markdown-insert-wiki-link :wk "insert wikilink")
    "iq" '(markdown-insert-blockquote :wk "insert block quote")
    "m" '(:ignore t :wk "move")
    "m M-j" '(markdown-move-subtree-down :wk "move subtree down")
    "m M-k" '(markdown-move-subtree-up :wk "move subtree up")
    "m C-k" '(markdown-move-list-item-down :wk "move list item down")
    "m C-k" '(markdown-move-list-item-up :wk "move list item up")
    "mj" '(markdown-move-down :wk "move down things at point")
    "mk" '(markdown-move-up :wk "move up things at point")
    "t" '(:ignore t :wk "toggle")
    "ti" '(markdown-toggle-inline-images :wk "toggle inline images")
    "tf" '(markdown-toggle-fontify-code-blocks-natively :wk "toggle fontify code")
    "tG" '(markdown-toggle-gfm-checkbox-button :wk "toggle gfm checkbox button")
    "tg" '(markdown-toggle-gfm-checkbox :wk "toggle gfm checkbox")
    "tM" '(markdown-toggle-markup-hiding :wk "toggle markup hiding")
    "tm" '(markdown-toggle-math :wk "toggle math")
    "tu" '(markdown-toggle-url-hiding :wk "toggle url hiding")
    "tw" '(markdown-toggle-wiki-links :wk "toggle wiki links"))

  (+config/local-leader
    :keymaps 'markdown-mode-map
    :states 'visual
    "r" '(:ignore t :wk "region actions")
    "rq" '(markdown-blockquote-region :wk "block quote region")
    "rp" '(markdown-pre-region :wk "pre region"))

  (general-nvmap
    :keymaps 'markdown-mode-map
    "gb" '(markdown-outline-previous-same-level :wk "previous same level")
    "gf" '(markdown-outline-next-same-level :wk "next same level")
    "gn" '(markdown-outline-next :wk "outline next")
    "gp" '(markdown-outline-previous :wk "outline previous")
    "z-" '(markdown-promote :wk "pronote")
    "z=" '(markdown-demote :wk "demote")
    "z<" '(markdown-outdent-region :wk "outdent region")
    "z>" '(markdown-indent-region :wk "indent region")
    "zj" '(markdown-move-down :wk "move up")
    "zk" '(markdown-move-up :wk "move up")
    "zt" '(:ignore t :wk "table")
    "ztj" '(markdown-table-insert-row :wk "insert row")
    "ztl" '(markdown-table-insert-column :wk "insert-column"))

  (general-def
    :keymaps 'markdown-mode-map
    "M-h" '(markdown-promote-subtree :wk "promote subtree")
    "M-S-h" '(markdown-promote :wk "promote")
    "M-l" '(markdown-demote-subtree :wk "demote subtree")
    "M-S-l" '(markdown-demote :wk "demote")))
#+end_src
*** Basic Configuration
#+begin_src emacs-lisp -n
(use-package markdown-mode
...
(use-package rustic
  ...
  (with-eval-after-load 'general
    (general-nvmap
      :keymaps '(rustic-mode-map lsp-mode-map)
      "K" 'lsp-describe-thing-at-point)))
#+end_src
...
#+end_src
* EXWM
For now, /EXWM/ only runs on /X11/.
** Script
:PROPERTIES:
:header-args: :tangle ~/.local/bin/startexwm :shebang #!/bin/sh :mkdirp t
:END:
This script will be called by the desktop file since I'm using a display manager. I put this in my ~~/.local/bin/~ directory, but if more than one users will be using /EXWM/, perhaps ~/usr/local/bin~ or even ~/usr/bin~ is better.
...
** the package
:PROPERTIES:
:header-args: :tangle ./modules/config-exwm.el :mkdirp t
:END:
*** header :noexport:
#+begin_src emacs-lisp +n
;;; config-exwm --- exwm configuration -*- lexical-binding: t -*-
;;; Commentary:
;;; Code:
#+end_src
*** =desktop-environment=
...
(use-package desktop-environment
  :ensure
  :commands (desktop-environment-music-next )
  :after exwm
  :init
...
(use-package exwm
  :ensure
  :demand t ; prevent automatic deferred loading.
  :preface ; runs before everything except :disabled.
  (menu-bar-mode -1)
  (tool-bar-mode -1)
  (scroll-bar-mode -1)
  (fringe-mode 1)
  ...
  :init ; runs before `exwm' is loaded.
  ...
  (defun +config-exwm--init-hook ()
    ...
    (+config-exwm--run-in-background "dbus-update-activation-environment --systemd --all")
    (+config-exwm--run-in-background "xrdb -merge ~/.Xresources")
    (+config-exwm--run-in-background "xsetroot -cursor_name left_ptr")
    (+config-exwm--run-in-background "xset r rate 300 30")
    (+config-exwm--run-in-background "nm-applet --indicator")
    (+config-exwm--run-in-background "/usr/lib/polkit-kde-authentication-agent-1")
    (+config-exwm--run-in-background "picom")
    (+config-exwm--run-in-background "udiskie -t")
    (+config-exwm--run-in-background "dunst -conf ~/.config/dunst/dunstrc")
    (+config-exwm--run-in-background "wal --theme solarized")
    (+config-exwm--run-in-background "blueman-applet")
    (+config-exwm--run-in-background "volumeicon")
    (+config-exwm--run-in-background "xsettingsd")
    (+config-exwm--run-in-background "unclutter")
    (+config-exwm--run-in-background "greenclip daemon>/dev/null")
    (+config-exwm--run-in-background "kdeconnect-indicator")
    (+config-exwm--run-in-background "/usr/lib/geoclue-2.0/demos/agent"))

  :config ; runs after `exwm' is loaded
  (defun +config-exwm--fix-exwm-floating-windows ()
  ...
  (defun +config-exwm--fix-exwm-title ()
    "Rename buffer name with exwm-title."
    (when (or (not exwm-instance-name)
              (string-prefix-p "sun-awt-X11-" exwm-instance-name)
              (string= "gimp" exwm-instance-name))
      (exwm-workspace-rename-buffer exwm-title)))

  (defun +config-exwm--rename-buffer-to-class-name ()
    "Rename workspace buffer to `exwm-class-name'."
    (exwm-workspace-rename-buffer exwm-class-name))

  (defmacro +config-exwm--raise-or-run (prog)
    "Run program or raise if already ran."
  ...
  (defun +config-exwm--rofi-launcher ()
    "Run rofi."
    (interactive)
    (+config-exwm--run-in-background "rofi -show drun -theme exwm"))

  (defun +config-exwm--greenclip-launcher ()
    "Run greenclip."
    (interactive)
    (+config-exwm--run-in-background "rofi-greenclip exwm"))

  (defun +config-exwm--setup-header-line-format ()
    "Hide header line if required."
    (set-window-parameter (next-window) 'header-line-format
                          (unless (window-at-side-p (next-window) 'top)
                            'none))) (defvar +config-exwm--exwm-workspace-switch-history-hack (cons exwm-workspace-current-index '()))

  (defun +config-exwm--set-current-workspace-index ()
    "Set index of workspace in +CONFIG-EXWM--EXWM-WORKSPACE-SWITCH-HISTORY-HACK.
Unless EXWM-WORKSPACE-CURRENT-INDEX is equal to +config-exwm--exwm-workspace-switch-history-hack."
    (unless (= exwm-workspace-current-index (car +config-exwm--exwm-workspace-switch-history-hack))
      (setq +config-exwm--exwm-workspace-switch-history-hack
            (cons exwm-workspace-current-index
                  (car +config-exwm--exwm-workspace-switch-history-hack)))))

  (defun +config-exwm--workspace-switch-to-last ()
    "Switch to last visited workspace."
    (interactive)
    "Switch to the workspace that was used before current workspace"
    (exwm-workspace-switch (cdr +config-exwm--exwm-workspace-switch-history-hack)))

  (defvar +config-exwm--simulation-key-window-classes
    '(google-chrome firefox qutebrowser))

  (defun +config-exwm--local-simulation-key ()
    "Simulation keys for certain classes."
    (when (and exwm-class-name
               (member exwm-class-name +config-exwm--simulation-key-window-classes))
  ...
  (setq exwm-manage-configurations
        ...
        '(((string= exwm-class-name "Xterm")
           char-mode t workspace 0)
          ((string= exwm-class-name "Termite")
           char-mode t workspace 0)
          ((string= exwm-class-name "URxvt")
           char-mode t workspace 0)
          ((string= exwm-class-name "Gnome-terminal")
           char-mode t workspace 0)        ;; windows 1 is exclusively for `Emacs'
          ...
          ((string= exwm-class-name "thunderbird")
           char-mode t workspace 3)
          ((string= exwm-class-name "Mail")
           char-mode t workspace 3)
          ;; workspace 4 is for files
          ((string= exwm-class-name "Thunar")
           char-mode t workspace 4)
          ;; workspace 5 for multimedia
          ((string= exwm-class-name "Spotify")
           char-mode t workspace 5)
          ((string= exwm-class-name "Cheese")
           char-mode t workspace 5)
          ((string= exwm-class-name "Picard")
           char-mode t workspace 5)
          ((string= exwm-class-name "hakuneko-desktop")
           char-mode t workspace 5)
          ...
          ((string= exwm-class-name "Gimp")
           char-mode t workspace 5)
          ;; workspace 6 for Network apps
          ((string= exwm-class-name "Org.remmina.Remmina")
           char-mode t workspace 6)
          ((string= exwm-class-name "Virt-manager")
           char-mode t workspace 6)
          ((string= exwm-class-name "Vncviewer")
           char-mode t workspace 6)
          ;; workspace 7 for Games
          ;; workspace 8 for work
          ((string= exwm-class-name "TelegramDesktop")
           char-mode t workspace 8)
          ((string= exwm-class-name "libreoffice-startcenter")
           char-mode t workspace 8)
          ((string= exwm-class-name "libreoffice-calc")
           char-mode t workspace 8)
          ((string= exwm-class-name "libreoffice-draw")
           char-mode t workspace 8)
          ((string= exwm-class-name "libreoffice-math")
           char-mode t workspace 8)
          ((string= exwm-class-name "libreoffice-writer")
           char-mode t workspace 8)
          ))

  (exwm-input-set-key (kbd "C-x C-c") '+config-exwm--exwm-logout)
  ...
  (setq exwm-input-global-keys
        `(
          ...
          ([?\s-q] . delete-window)
          ([?\s-j] . perspective-exwm-cycle-exwm-buffers-forward)
          ([?\s-J] . perspective-exwm-cycle-all-buffers-forward)
          ([?\s-k] . perspective-exwm-cycle-exwm-buffers-backward)
          ([?\s-K] . perspective-exwm-cycle-all-buffers-backward)
          ...
          ([?\s-d] . +config-exwm--rofi-launcher)
          ([?\s-c] . +config-exwm--greenclip-launcher)
  ...
  (require 'exwm-randr)
  (cond ((string= "somalia-minimal" system-name)
         (progn
           (setq exwm-randr-workspace-monitor-plist
                 '(0 "eDP1" 1 "eDP1" 2 "eDP1" 3 "eDP1" 4 "HDMI-1"
                     5 "HDMI-1" 6 "HDMI-1" 7 "HDMI-1" 8 "HDMI-1" 9 "HDMI-1")))))
  (add-hook 'exwm-randr-screen-change-hook
            (lambda ()
              (+config-exwm--run-in-background "~/.fehbg")))
  ;; (exwm-randr-refresh)
  (exwm-randr-enable)
  ...
  ;; Initialize EXWM
  :hook
  (exwm-init . +config-exwm--init-hook)
  (exwm-init . exwm-modeline-mode)
  ;; Normal hook run when an X window has been made floating
  (exwm-floating-setup . +config-exwm--fix-exwm-floating-windows)
  (exwm-floating-setup . exwm-layout-hide-mode-line)
  ;; Normal hook run when window title is updated.
  (exwm-update-title . +config-exwm--fix-exwm-title)
  ;; Normal hook run when window class is updated.
  (exwm-update-class . +config-exwm--rename-buffer-to-class-name)
  ;; normal hook run after switching workspace.
  (exwm-workspace-switch . +config-exwm--set-current-workspace-index)
  (window-configuration-change . +config-exwm--setup-header-line-format)
  (exwm-manage-finish . +config-exwm--local-simulation-key))
#+end_src
*** =exwm-systemtray=
#+begin_src emacs-lisp
...
* Footnotes
[fn:18] https://www.emacswiki.org/emacs/SelfDocumentation
