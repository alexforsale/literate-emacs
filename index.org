...
#+begin_src emacs-lisp
  (use-package evil-easymotion
    ...
    (evilem-make-motion evilem-motion-backward-WORD-end #'evil-backward-WORD-end :scope 'visible)
    (evilem-default-keybindings "z f")
    )
#+end_src
This is a useful package for complementing /Evil/'s motion. With =general.el=, this is the keybinding (using ~zf~), so the fastest motion to use is ~zff~ for .
| evilem keybinding                   | evilem key | Evil                      | evil key |
|-------------------------------------+------------+---------------------------+----------|
| =evilem-motion-forward-word-begin=  | ~z f w~    | =evil-forward-word-begin= | ~w~      |
| =evilem-motion-forward-WORD-begin=  | ~z f W~    | =evil-forward-WORD-begin= | ~W~      |
| =evilem-motion-forward-word-end=    | ~z f e~    | =evil-forward-word-end=   | ~e~      |
| =evilem-motion-forward-WORD-end=    | ~z f E~    | =evil-forward-WORD-end=   | ~E~      |
| =evilem-motion-backward-word-begin= | ~z f b~    | =evil-forward-word-begin= | ~b~      |
| =evilem-motion-backward-WORD-begin= | ~z f B~    | =evil-forward-WORD-begin= | ~B~      |
| =evilem-motion-backward-word-end=   | ~z f g e~  | =evil-forward-word-end=   | ~g e~    |
| =evilem-motion-backward-WORD-end=   | ~z f g E~  | =evil-forward-WORD-end=   | ~g E~    |
...
#+begin_src emacs-lisp
  (use-package evil-nerd-commenter
    ...
    (evil-define-key* nil my/evilnc-map "c" '("comment/uncomment" . evilnc-comment-or-uncomment-lines))
    (evil-define-key* nil my/evilnc-map "o" '("copy and paste commented" . evilnc-copy-and-comment-lines)))
#+end_src
...
+Adds standard vim keybindings of ~zf~ and ~zd~ to create and delete folds (via =vimish-fold=) respectively+. Fold now can be created with =<leader>zff=, after that, it hooks into evil so the usual vim keybindings for fold toggling (~za~), opening (~zo~), closing (~zc~) etc all work as expected with vimish-fold.
...
#+begin_src emacs-lisp
  (use-package evil-vimish-fold
    ...
    (evil-define-key nil my/z-map "f" (cons "fold" my/vimish-fold-map))
    (evil-define-minor-mode-key nil my/vimish-fold-map
      "j" '("next fold" . vimish-fold-next-fold)
      "k" '("prev fold" . vimish-fold-previous-fold)
      "f" '("create" . evil-vimish-fold/create)
      "F" '("create line" . evil-vimish-fold/create-line)
...
#+begin_src emacs-lisp
  ...
  ;;;; 5. No project support
    ...
    (evil-define-key* '(normal visual) 'global (kbd "<leader>.") '("Store register" . consult-register-store))
    (evil-define-key* nil my/buffer-map
...
*** [[https://github.com/radian-software/apheleia][apheleia]] :external:
#+begin_src emacs-lisp
...
#+end_src
*** [[https://github.com/magnars/expand-region.el][expand-region]] :external:
...
#+name: git-timemachine :tangle no
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (when (file-directory-p (expand-file-name "roam" org-directory))
...
#+begin_src emacs-lisp
  (use-package org-refile
    ...
    :custom
    (org-refile-targets
     `((,(expand-file-name "projects.org" org-directory) :maxlevel . 2)
       (,(expand-file-name "notes.org" org-directory) :maxlevel . 1)
       (,(expand-file-name "routines.org" org-directory) :maxlevel . 2)
...
#+end_src
*** org-roam
I've tried different Emacs packages for note taking. Although this is not a lightweight solution, but sorry to say it's still the best one for me. And I publish it to my [[https://alexforsale.github.io/braindump/][personal blog]]. This time I wont do that though
#+name: org-roam
#+begin_src emacs-lisp
...
#+end_src
*** [[https://gitlab.com/hperrey/khalel/][Khalel]]
...
#+begin_src conf :tangle no
  ...
  default_calendar = *
#+end_src
*** [[https://github.com/DamienCassou/khardel][khardel]]
