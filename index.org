...
#+begin_src emacs-lisp -n
  ...
  (custom-set-variables
   ...
   '(use-package-check-before-init nil))
#+end_src
*** footer                                                       :noexport:
#+begin_src emacs-lisp -n
  (require 'use-package)
...
**** emms
#+begin_src emacs-lisp -n
  (general-def 'emms-browser-mode-map
    "?" 'describe-mode
...
*** example
**** Simple
#+begin_src plantuml :exports both :file ./assets/img/ping.svg
Client -> Server : Ping
Server -> Client : Pong
#+end_src

#+RESULTS:
[[file:./assets/ping.svg]]

**** gerrit
#+BEGIN_SRC plantuml :exports results :file ./assets/img/git-gerrit-workflow.png
title Git + Gerrit Work Flow

(*) -down-> "clone repository"
note right
git clone url/to/repository
scp -p url/to/hooks/commit-msg repository/.git/hooks/
endnote
-down->[get an assignment] "coding" as coding
-down-> "commit locally"
note right
git add xxx
git commit
endnote
-down-> "review" as review
note right: git push origin refs/for/<branch_name>
--> if "" then
-right->[approved] "submit"
note bottom: click "submit" on corresponding gerrit page
-u-> "update local repository"
note right: git pull <color:red>--rebase</color>
...
note top
git add xxx
git commit <color:red>--amend</color>
endnote
-right-> review
endif
#+END_SRC
...
#+RESULTS:
[[file:./assets/img/git-gerrit-workflow.png]]
...
#+begin_src diagrams :type flowchart :exports both
  st=>start: Incoming Tasks
  e=>end
