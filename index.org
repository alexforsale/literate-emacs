...
#+begin_src emacs-lisp
  (use-package python
    ...
    (when (featurep 'lsp-mode)
      (setopt lsp-pylsp-plugins-rope-completion-enabled t ; needs python-rope package
            lsp-pylsp-plugins-mccabe-enabled t ; needs python-mccabe package
            lsp-ruff-lsp-python-path (executable-find "python3"))
      (when (executable-find "black")
        (setopt lsp-pylsp-plugins-black-enabled t))
      (when (executable-find "autopep8")
        (setopt lsp-pylsp-plugins-autopep8-enabled t))
      (when (executable-find "flake8")
        (setopt lsp-pylsp-plugins-flake8-enabled t))
      (when (executable-find "pycodestyle")
        (setopt lsp-pylsp-plugins-pycodestyle-enabled t))
      (when (executable-find "pydocstyle")
        (setopt lsp-pylsp-plugins-pydocstyle-enabled t))
      (when (executable-find "pylint")
        (setopt lsp-pylsp-plugins-pylint-enabled t))
      (when (executable-find "pyflakes")
        (setopt lsp-pylsp-plugins-pyflakes-enabled t))
      (when (executable-find "yapf")
        (setopt lsp-pylsp-plugins-yapf-enabled t)))
...
#+begin_src emacs-lisp
  (use-package toml-mode
    ...
    (with-eval-after-load 'lsp-mode
      ...
      (setopt lsp-toml-command (executable-find "taplo"))
