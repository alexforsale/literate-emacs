...
#+end_src
*** [[https://github.com/hlissner/evil-snipe][evil-snipe]] :external:archive:
...
#+begin_src emacs-lisp :noweb-ref minimal-config :noweb-sep "\n\n" :tangle no
  (use-package emacs
    ...
    (setopt minibuffer-prompt-properties '(read-only t intangible t cursor-intangible t face minibuffer-prompt))
...
#+begin_src emacs-lisp :noweb-ref minimal-config :noweb-sep "\n\n"
  (use-package repeat
    ...
    (repeat-mode +1))
#+end_src
*** [[https://github.com/justbur/emacs-which-key][which-key]] :builtin:
  ...
  (use-package ispell
    ...
    (when (executable-find "aspell")
      ...
      (unless ispell-aspell-dict-dir
        (setopt ispell-aspell-dict-dir
              (ispell-get-aspell-config-value "dict-dir")))
      (unless ispell-aspell-data-dir
        (setopt ispell-aspell-data-dir
...
#+end_src
*** consult-eglot :external:archive:
#+begin_src emacs-lisp :tangle no
  (use-package consult-eglot
    :after '(consult eglot)
    :ensure t)
#+end_src
*** consult-eglot-embark :external:archive:
:PROPERTIES:
...
#+begin_src emacs-lisp
  (use-package lsp-mode
  	:ensure t
  	:hook ((nix-ts-mode lua-ts-mode python-ts-mode
  						yaml-ts-mode toml-ts-mode
  						bash-ts-mode js-ts-mode) . lsp-deferred)
...
#+begin_src emacs-lisp
  (use-package elfeed-org
    ...
    :config
    (elfeed-org)
    (setopt rmh-elfeed-org-files (list (expand-file-name "elfeed.org" org-directory))))
#+end_src
...
#+begin_src emacs-lisp :tangle no
  ...
  (use-package mastodon
    ...
    (mastodon-discover)
    (with-eval-after-load 'evil
  	(setopt my/mastodon-map (make-sparse-keymap))
...
*** tab-width function
#+begin_src emacs-lisp
  (defun my/two-spaces-soft-tabs ()
    (setq-local tab-width 2))

  (defun my/four-spaces-soft-tabs ()
    (setq-local tab-width 2))
#+end_src
**** For misc languages/modes
#+begin_src emacs-lisp
  ;; yaml
  (add-hook 'yaml-ts-mode-hook #'my/two-spaces-soft-tabs)
#+end_src
*** javascript :builtin:
#+begin_src emacs-lisp
  (use-package js
    ...
    :mode ("\\.es6\\'" . js-ts-mode)
    :mode ("\\.pac\\'" . js-ts-mode)
    :hook (js-ts-mode . my/two-spaces-soft-tabs)
    :config
...
#+begin_src emacs-lisp
  (use-package typescript-mode
    :ensure t
    :hook (typescript-ts-mode my/two-spaces-soft-tabs)
    :interpreter "ts-node")
#+end_src
**** rjsx :external:
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package markdown-mode
  	...
  	(when (executable-find "markdown")
  		(setopt markdown-command "markdown"))
...
#+begin_src emacs-lisp
  (use-package python
    ...
    :hook (python-ts-mode . my/four-spaces-soft-tabs)
    :config
    (with-eval-after-load 'evil-collection
      (evil-collection-python-setup))
    (if (executable-find "ipython")
        (setopt python-interpreter (executable-find "ipython"))
      (setopt python-interpreter (executable-find "python3")))
    ...
    (when (featurep 'flycheck)
      (setopt flycheck-python-mypy-executable (executable-find "mypy")
        ...
        (save-excursion
          ...
          (save-match-data
            (when (or (looking-at "#!/usr/bin/env \\(python[^ \n]+\\)")
                      (looking-at "#!\\([^ \n]+/python[^ \n]+\\)"))
              (setopt executable (substring-no-properties (match-string 1))))))
    ...
    (defun +python-pyenv-mode-set-auto-h ()
      ...
      (when (eq major-mode 'python-mode)
        (when (not (local-variable-p '+pyenv--version))
          ...
          (setopt +pyenv--version (+python-pyenv-read-version-from-file)))
        (if +pyenv--version
    ...
    (with-eval-after-load 'evil
        (setopt my/python-map (make-sparse-keymap))
      ...
      (setopt my/python-send-map (make-sparse-keymap))
      (setopt my/python-repl-map (make-sparse-keymap))
...
#+begin_src emacs-lisp
  ...
  (use-package rustic
    :ensure t
    :after rust-mode
    :hook (rustic-mode . my/four-spaces-soft-tabs)
    :init
    ...
    (when (executable-find "rustup")
      (setopt rustic-analyzer-command
            `(,(executable-find "rustup")
              "run"
              "stable"
              "rust-analyzer")))
    (setopt rustic-indent-method-chain t)
    (setopt rustic-babel-format-src-block nil
          rustic-format-trigger nil)
    (with-eval-after-load 'evil
      (setopt my/rustic-map (make-sparse-keymap))
      (setopt my/cargo-map (make-sparse-keymap))
      ...
      (evil-define-key* nil my/rustic-map
  	  (kbd "c") (cons "Cargo" my/cargo-map)
  	  (kbd "p") '("rustic popup" . rustic-popup))
...
#+begin_src emacs-lisp :noweb-ref minimal-config :noweb-sep "\n\n"
  (use-package dired
    ...
    :init
    (setopt dired-dwim-target t ; guess a default target directory
...
#+begin_src emacs-lisp
  (use-package dired-git-info
    ...
    :config
    ;; (setopt +dired--git-info-p (bound-and-true-p dired-git-info-mode))
